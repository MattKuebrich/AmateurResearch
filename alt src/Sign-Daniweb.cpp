//this tries to replicate the code posted here...
...
//from the daniweb example, this is the 10K buffer the poster wanted.
//this basically makes phase go from smoothly go from 0 to 1 to 0 to 1, and so on... otherwise it the osc gets all stairsteppy
		if (phase >= 10000.0f) {   //if phase is more or equal to 1
				//phase = phase - 1.0f;
				phase = phase - 10000.0f;  /// once phase = 1, this subtracts 1, bringing phase back to zero
				// have the - 1.0f value be CV controlled?
		}
		//printf("%f\n",phase);

	//float sine = sin(2*M_PI*phase);
	float sine = sin(2*M_PI*freq2 * phase/sampleRate);
